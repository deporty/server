import { PermissionEntity, ResourceEntity, RoleEntity } from '@deporty-org/entities/authorization';
...
import { AUTHORIZATION_SERVER, BEARER_TOKEN } from '../users.constants';

export class AuthorizationRepository extends AuthorizationContract {
  getPermissionById(permissionId: string): Observable<PermissionEntity> {
    throw new Error('Method not implemented.');
  }
  getResourceById(resourceId: string): Observable<ResourceEntity> {
    throw new Error('Method not implemented.');
  }
  ...
  getRoleById(roleId: string): Observable<RoleEntity> {
    return new Observable((observer) => {
      axios
        .get<IBaseResponse<RoleEntity>>(`${AUTHORIZATION_SERVER}/${roleId}`, {
          headers: {
            Authorization: `Bearer ${BEARER_TOKEN}`,
          },
        })
