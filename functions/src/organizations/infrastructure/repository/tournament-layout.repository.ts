...
import {
  ...
  TOURNAMENT_LAYOUTS_ENTITY,
} from '../organizations.constants';
import {
  AccessParams,
  TournamentLayoutContract,
} from '../../domain/contracts/tournament-layout.contract';
...
export class TournamentLayoutsRepository extends TournamentLayoutContract {
  ...
  constructor(
    ...
    protected mapper: TournamentLayoutMapper
  ) {
  ...
  getById(
    ...
    id: Id
  ): Observable<TournamentLayoutEntity | undefined> {
  ...
  get(
    ...
    return super.innerGet(
      [
        { collection: ORGANIZATIONS_ENTITY, id: accessParams.organizationId },
        { collection: TOURNAMENT_LAYOUTS_ENTITY },
      ],
      pagination
    );
  }
  ...
  filter(
    accessParams: AccessParams,
    filter: Filters
  ): Observable<Array<TournamentLayoutEntity>> {
