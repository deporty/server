...
import {
  AccessParams,
  NodeMatchContract
} from '../../domain/contracts/node-match.contract';
...
export class NodeMatchRepository extends NodeMatchContract {
  ...
  constructor(
    ...
    protected groupMapper: NodeMatchMapper
  ) {
  ...
  getById(
    ...
    id: string
  ): Observable<NodeMatchEntity | undefined> {
  ...
  ): Observable<NodeMatchEntity[]> {
    return super.innerGet(
      [
        { collection: TOURNAMENTS_ENTITY, id: accessParams.tournamentId },
        { collection: MAIN_DRAW_ENTITY },
      ],
      pagination
    );
  }
