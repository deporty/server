    ...
    makeMatrix(param, teamIds) {
        return this.getGroupMatchesUsecase
            ...
            if (!areAllMatchesSetted) {
                ...
                console.log('Hola', teamIds.length);
                ...
                for (const match of matches) {
                    ...
                    matrix.table[indexTeamA] = Array.from({
                        length: teamIds.length,
                    });
                    matrix.table[indexTeamB] = Array.from({
                        length: teamIds.length,
                    });
                    console.log('Gordita');
                    console.log(matrix.table);
                    ...
                    console.log('Nuew Table ');
                    console.log(matrix.table);
                    console.log();
                }
                const newMatches = [];
                for (let row = 1; row < teamIds.length; row++) {
                    for (let column = 0; column < teamIds.length - 1; column++) {
                        if (row != column) {
                            console.log('Row: ', row, ' column: ', column);
                            const existsMatch = matrix.table[row][column];
                            console.log('Existe? ', existsMatch);
                            if (!existsMatch) {
